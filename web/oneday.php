...
try {
    ...
    if($lastHour != $currentHour) {
        ...
        $data_day = ['temps' => [], 'hemis' => [],'pressures' => [], 'times' => []];
        
        // Заполнение массивов
        foreach ($rows_day as $row_day) {
            $data_day['temps'][] = $row_day['temp'];
			$data_day['hemis'][] = $row_day['hemi'];
        ...
        $avg_hemi_hour = array_sum($data['hemis']) / count($data['hemis']);
